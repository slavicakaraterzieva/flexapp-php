...
//validate category subcategory occupation

public function checkJob($validate_category){
  $this->db->query('SELECT job_category_title FROM job_category WHERE job_category_title =:job_category_title');
  //bind params
  $this->db->bind(':job_category_title',$validate_category);
$row = $this->db->single();
return $row;
}


public function checkSub($validate_subcategory){
  $this->db->query('SELECT specific_sub_title FROM specific_sub WHERE specific_sub_title =:specific_sub_title');
  //bind params
  $this->db->bind(':specific_sub_title',$validate_subcategory);
$row = $this->db->single();
return $row;
}

public function checkOcc($validate_occupation){
  $this->db->query('SELECT occupation_title FROM occupation WHERE occupation_title =:occupation_title');
  //bind params
  $this->db->bind(':occupation_title',$validate_occupation);
$row = $this->db->single();
return $row;
}
...
public function getJobCatId(){
  $this->db->query('SELECT job_category_id FROM `job_category` ORDER BY job_category_id DESC LIMIT 1');
  $result =$this->db->single();
  return $result;
}

// inserting new category
public function newCategory($new_cat, $parent_id){
  $this->db->query('INSERT INTO `job_category`(job_category_title, parent_id) 
  VALUES(:job_category_title, :parent_id)');
  $this->db->bind(':job_category_title',$new_cat);
  $this->db->bind(':parent_id',$parent_id);
  if($this->db->execute()){
    return true;
    }
...
//insert new sub
public function addSub($sub_name, $last_id){
  $this->db->query('INSERT INTO `specific_sub`(specific_sub_title, parent_sub_id) 
  VALUES(:specific_sub_title, :parent_sub_id)');
  $this->db->bind(':specific_sub_title',$sub_name);
  $this->db->bind(':parent_sub_id',$last_id);
  if($this->db->execute()){
    return true;
    }
    else{
...
public function addOccupation($occupation_name, $last_id){
  $this->db->query('INSERT INTO `occupation`(occupation_title, parent_occupation_id) 
  VALUES(:occupation_title, :parent_occupation_id)');
  $this->db->bind(':occupation_title',$occupation_name);
  $this->db->bind(':parent_occupation_id',$last_id);
  if($this->db->execute()){
    return true;
    }
    else{
